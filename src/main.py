import cv2,mediapipe as mp,numpy as np, tkinter, random, time, pyfiglet
from tkinter import *  
result = pyfiglet.figlet_format("Aakarshit  Rekhi")
print(result)
time.sleep(1)
...
def controller():
  try:
    ...
    tk.title("Real Time Body pose estimator - Projector")
    canvas.pack()
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
      while cap.isOpened():
        ...
        canvas.create_arc(smile_x1, smile_y1, smile_x2, smile_y2, start =0, extent = -180, fill = "brown")
        ...
        L_eye = canvas.create_oval(L_eye_x1, L_eye_y1, L_eye_x2, L_eye_y2, fill="black")
        R_eye = canvas.create_oval(R_eye_x1, R_eye_y1, R_eye_x2, R_eye_y2, fill="black")
        ...
        cv2.imshow('Real Time Body pose estimator - Controller', image)
...
app=Tk()
app.state("zoomed")
app.title("Real Time Body pose estimator - Menu")
img = ImageTk.PhotoImage(Image.open("qpgb39.jpg"))
panel = Label(app, image = img)
panel.pack(side = "bottom", fill = "both", expand = "yes")

title=Label(app,text="Real Time \nBody Pose Estimator",font=("Comic Sans MS",30),bg="#F4F973").place(x=50,y=100)
start=Button(app, text="Open Webcam",font=("Arial",15),bd=0,bg="#ADD8E6",width=20,command=controller).place(x=130,y=280)
start=Button(app,command=githubrepo, text="Github Repo",font=("Arial",15),bd=0,bg="#00FFFF",width=20).place(x=130,y=330)
start=Button(app,command=close, text="Exit",font=("Arial",15),bd=0,bg="#FFC0CB",width=20).place(x=130,y=380)
app.mainloop()
